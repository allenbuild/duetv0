...
def build_parser() -> argparse.ArgumentParser:
    ...
    c.add_argument("--zed", action="append", default=[], metavar="NAME=EXPORT_DIR_OR_ZIP", help="ZED export (folder or zip from zed_kit.py / zed_mac_record.py); becomes an ego stream (repeatable)")
...
def cmd_create(a: argparse.Namespace) -> int:
    ...
    for zname, zrole, zpath, zperson in (parse_stream(x, "ego") for x in a.zed):  # unpack first: create keeps zed/ and links left.mp4
        from duet.playground import zed_bundle
        vids.append((zname, zrole, zed_bundle.ingest(zpath.expanduser(), Path(episodes_root(a)) / a.name, zname), zperson))
